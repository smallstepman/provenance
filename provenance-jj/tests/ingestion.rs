...
use provenance_jj::{jj_commit_address, jj_operation, observe_repository, why_jj_commit};
...
fn repeated_observation_is_idempotent() {
    ...
    let published_after_first = service.runtime.published_len();

    fixture.ingest(&mut service);

    assert_eq!(service.state, state_after_first);
    assert_eq!(service.runtime.published_len(), published_after_first);
    assert_eq!(
