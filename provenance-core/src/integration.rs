...
pub enum ProcessError<A, R> {
    ...
    Adapter(#[source] A),
    #[error("provenance core rejected transaction: {0}")]
    Core(#[source] Error),
...
/// Errors raised after an already-converted transaction enters the core
/// planning and runtime commit protocol.
///
/// Unlike [`ProcessError`], this type has no adapter stage because callers
/// supply a transaction directly.
#[derive(Debug, ThisError, Serialize, Deserialize)]
pub enum TransactionProcessError<R> {
    #[error("provenance core rejected transaction: {0}")]
    Core(#[source] Error),
    #[error("prepare stage failed: {0}")]
    Prepare(#[source] R),
    #[error("publish stage failed: {0}")]
    Publish(#[source] R),
    /// IMPORTANT: Operation is already authoritative when this is returned.
    #[error("finalize stage failed: {0}")]
    Finalize(#[source] R),
}

impl<A, R> From<TransactionProcessError<R>> for ProcessError<A, R> {
    fn from(error: TransactionProcessError<R>) -> Self {
        match error {
            TransactionProcessError::Core(error) => Self::Core(error),
            TransactionProcessError::Prepare(error) => Self::Prepare(error),
            TransactionProcessError::Publish(error) => Self::Publish(error),
            TransactionProcessError::Finalize(error) => Self::Finalize(error),
        }
    }
}
    ...
    RT: Runtime<M>,
{
    ...
    /// Process a transaction produced by an integration other than the
    /// service's primary adapter.
    ///
    /// The transaction still uses the same kernel planning and runtime commit
    /// protocol as transactions produced by [`Adapter::transaction`].
    #[tracing::instrument(level = "debug", skip_all)]
    pub fn process_transaction(
        &mut self,
        transaction: Transaction<M>,
    ) -> std::result::Result<(), TransactionProcessError<RT::Error>> {
        let plan = self
            .kernel
            .transact(&self.state, transaction)
            .map_err(TransactionProcessError::Core)?;
    ...
    fn commit_plan(
        ...
        for requirement in &plan.prepare {
            self.runtime
                .prepare(requirement)
                .map_err(TransactionProcessError::Prepare)?;
        }

        self.runtime
            .publish(&plan.operation)
            .map_err(TransactionProcessError::Publish)?;
        ...
        for action in &plan.finalize {
            self.runtime
                .finalize(action)
                .map_err(TransactionProcessError::Finalize)?;
        }
...
mod tests {
    ...
    #[test]
    fn direct_transaction_error_has_no_adapter_stage() {
        let error = TransactionProcessError::Publish(TestError);

        assert_eq!(error.to_string(), "publish stage failed: disk full");
        assert_eq!(
            std::error::Error::source(&error)
                .expect("transaction process error source")
                .to_string(),
            "disk full"
        );
    }
